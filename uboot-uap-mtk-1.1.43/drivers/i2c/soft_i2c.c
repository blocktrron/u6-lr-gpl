...
#  define I2C_INIT \
	do { \
        mt_set_gpio_mode(GPIO14, GPIO_MODE_01);\
        mt_set_gpio_mode(GPIO15, GPIO_MODE_01); \
...
# ifndef I2C_READ
#  define I2C_READ mt_get_gpio_in(GPIO14)
# endif
...
#  define I2C_SDA(bit) \
	do { \
        if (bit) { \
            mt_set_gpio_dir(GPIO14, GPIO_DIR_IN);\
            mt_get_gpio_in(GPIO14); \
        } else { \
            mt_set_gpio_dir(GPIO14, GPIO_DIR_OUT);\
            mt_set_gpio_out(GPIO14, 0); } \
            I2C_GPIO_SYNC; \
	} while (0)
# endif
