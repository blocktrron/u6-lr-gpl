...
int rt2880_mdio_access45(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u32 addr;
	u32 phy_addr;
	u32 port_num = 1;
	...
	if(!memcmp(argv[0],"mdio45.r",sizeof("mdio45.r")))
	{
		if (argc != 4) {
			printf ("Usage:\n%s\n", cmdtp->usage);
			return 1;
		}
		port_num = simple_strtoul(argv[1], NULL, 10);
		phy_addr = simple_strtoul(argv[2], NULL, 10);
		addr = simple_strtoul(argv[3], NULL, 10);
		phy_addr &=0x1f;

		if(mii_mgr_read_cl45(port_num,phy_addr, addr, &value))
			printf("\n mdio45.r port %x addr[0x%08X]=0x%08X\n",port_num,addr,value);
		else {
			printf("\n Read addr[0x%08X] is Fail!!\n",addr);
		}
	}

	else if(!memcmp(argv[0],"mdio45.w",sizeof("mdio45.w")))
	{
		if (argc != 5) {
			printf ("Usage:\n%s\n", cmdtp->usage);
			return 1;
		...
		if(mii_mgr_write_cl45(port_num,phy_addr, addr, value)) {
			printf("\n mdio45.w addr[0x%08X]  value[0x%08X]\n",addr,value);
		}
		else {
...
U_BOOT_CMD(
	mdio45,	5,	1,	rt2880_mdio_access45,
	"mdio45   - Mediatek PHY register R/W command via CL45 !!\n",
	"mdio45.r [phy_addr(dec)] [reg_addr(dec)] \n"
	"mdio45.w [phy_addr(dec)] [reg_addr(dec)] \n"
);
