...
#define UBNT_BOARD_ID_U6LR              0x20
...
static int ubnt_read_kernel_image() {
...
static int ubnt_update_uboot_image()
{
...
static int ubnt_update_bootselect()
{
    return ubnt_update_partition(
...
static int ubnt_set_ble_mode_env(void) {
    ...
    if (ubnt_board_id[0] == UBNT_BOARD_AX_ID_PREFIX) {
        switch (ubnt_board_id[1]) {
        case UBNT_BOARD_ID_U6LR:
            setenv("device_model", "U6-LR");
            break;
        default:
            setenv("device_model", "None");
            return -1;
        }
        return 0;
    }
