...
void create_bound_listeners(int dienow)
{
  ...
  for (iface = daemon->interfaces; iface; iface = iface->next)
	...
	iface->done = 1;
        /* 
         * [UAP-1052]
         * We check if dnsmasq binded to ipv4 socket,
         * or client will not be directed to guest portal
         */
    ...
    if (dienow && !bind_ipv4) {
        my_syslog(LOG_WARNING, _("failed to bind ipv4 socket! Go die!"));
        die(_("failed to bind ipv4 socket! %s"), NULL, EC_BADNET);
    }
