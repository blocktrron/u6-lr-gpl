...
#define ENCRYPT_DEBUG_FILE "/tmp/syslogd_dbg.txt"
...
enum {
	...
	IF_FEATURE_REMOTE_LOG(    OPTBIT_remoteprefix,)	// -p
	IF_FEATURE_REMOTE_LOG(    OPTBIT_encrypt    ,)	// -E
	...
	OPT_remoteprefix= IF_FEATURE_REMOTE_LOG(    (1 << OPTBIT_remoteprefix)) + 0,
	OPT_encrypt     = IF_FEATURE_REMOTE_LOG(    (1 << OPTBIT_encrypt    )) + 0,
...
#define OPTION_STR "m:nO:l:S" \
	...
	IF_FEATURE_REMOTE_LOG(    "p:" ) \
	IF_FEATURE_REMOTE_LOG(    "E:" ) \
...
#define OPTION_PARAM &opt_m, &(G.logFile.path), &opt_l \
	...
	IF_FEATURE_REMOTE_LOG(    ,&G.prefixRemoteLogStr) \
	IF_FEATURE_REMOTE_LOG(    ,&G.encryptKey) \
...
#endif /* defined(__UBNT_LIBC_MUSL__) */
	timestamp[15] = '\0';
...
static int add_log_prefix(char *outbuf, int outlen, char *prefix, char *inbuf, int inlen)
{
	char *p, *tailp;
	int header_len, written, outleft;
	...
	if (DEBUG) {
		printf("%s before: %.*s", __FUNCTION__, inlen, inbuf);
	}
	...
	if ((tailp - p) > 16) {
		...
		if (p[3] != ' ' || p[6] != ' '
			...
			if (DEBUG) {
				printf("%s invalid timestamp: %.*s", __FUNCTION__, 16, p);
			}
		} else {
			memcpy(p, timestamp, 15); /* override the timestamp */
			p += 16;
		}
#else
		p += 16;
	...
	memcpy(outbuf, inbuf, header_len);
	written = header_len;
	outleft -= header_len;
	inlen -= header_len;
	written += snprintf(outbuf + written, outleft, "%s: ", prefix);
	if (written >= outlen) {
		written = outlen;
	} else {
		outleft = outlen - written;
		if (inlen > outleft)
			inlen = outleft;
		memcpy(outbuf + written, inbuf + header_len, inlen);
		written += inlen;
	}
	...
	if (DEBUG) {
		printf("%s after : %.*s", __FUNCTION__, written, outbuf);
	}
...
static int ui_set_hashid_in_aad(void)
{
	FILE *fp;
	char string[256];
	const char searchstr[] = "device.hashid=";
	const char *fieldptr;
	...
	while (fscanf(fp, "%s", string) == 1) {
		if (strncmp(string, searchstr, sizeof(searchstr) - 1) == 0) {
			fieldptr = strchr(string, '=');
			fieldptr++;

			for (count = 0;
					(count < HASH_ID_SIZE) &&
					(sscanf(fieldptr, "%2hhx", &hash_id[count]) > 0);
					count++) {
				fieldptr += 2;
				sum = hash_id[count];
			}
		...
		unsigned char *cipher_out, int cipher_out_len)
{
	...
	out_len += out_len_tmp; // should always be less than EVP_MAX_BLOCK_LENGTH
...
static void do_syslogd(void)
{
	...
	int len;
	char *sendbuf;
	int ui_encrypt_ready = 0;
	unsigned char buf_enc[MAX_READ + sizeof(gcm_aad) + sizeof(gcm_iv) + UI_AES_GCM_TAG_SIZE];
#endif
...
#define recvbuf (G.recvbuf)
	...
	ENCRYPT_DEBUG("option_mask32:%x OPT_encrypt:%x\n", option_mask32, OPT_encrypt);
...
#if ENABLE_FEATURE_REMOTE_LOG
	if (option_mask32 & OPT_encrypt) {
		ui_encrypt_ready = ui_encrypt_init(G.encryptKey) == 0;
	}
#endif
 ...
 read_again:
		...
		for (item = G.remoteHosts; item != NULL; item = item->link) {
			...
			if (G.prefixRemoteLogStr) {
				len = add_log_prefix(G.printbuf, MAX_READ, G.prefixRemoteLogStr, recvbuf, sz + 1);
				sendbuf = G.printbuf;
			} else {
				len = sz + 1;
				sendbuf = recvbuf;
			 ...
			 */
			if (len && sendto(rh->remoteFD, sendbuf, len,
