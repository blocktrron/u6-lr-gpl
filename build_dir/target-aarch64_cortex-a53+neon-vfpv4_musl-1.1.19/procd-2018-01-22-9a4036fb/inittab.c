...
#define PROCESS_TERMINATION_TIMEOUT	5000
#define MCAD_RESPAWN_TIMEOUT		10000
...
static void child_exit(struct uloop_process *proc, int ret)
{
	...
	struct timespec tp;
	long runtime;
	int send_alert = WEXITSTATUS(ret) && !(WIFSIGNALED(ret) || WIFSTOPPED(ret) || WIFCONTINUED(ret));

	get_action_command_string(command, sizeof(command), a);
	clock_gettime(CLOCK_MONOTONIC, &tp);
	runtime = tp.tv_sec - a->start.tv_sec;
	clock_gettime(CLOCK_MONOTONIC, &a->start);

	if (a->dispose) {
		DEBUG(4, "Process '%s' (pid %d, uid %d, pending %d) terminated with status %d.",
			command, proc->pid, a->unique_id, proc->pending, ret);
		free_action(a);
	} else {
		if (send_alert) {
			...
			a->_stdout.fd.fd = -1;
			/* Rate limit process restart events exponentially (send alert on 1,2,4,9,... crash) */
			if (a->alert_count * a->alert_count > a->crash_count)
				send_alert = 0;
		}

		DEBUG(0, "Process '%s' (pid %d, uid %d, pending %d, crashes %d) exited after %ld seconds with status %d. Scheduling for restart (%s).",
			command, proc->pid, a->unique_id, proc->pending, a->crash_count, runtime, ret, send_alert ? "alert sent" : "no alert sent");

		free_ustream(a);

		if (strstr(command, "mcad") && send_alert) {
			send_alert = 0;
			uloop_timeout_set(&a->tout, MCAD_RESPAWN_TIMEOUT);
		} else {
			uloop_timeout_set(&a->tout, a->respawn);
		}

		if (send_alert) {
			/* In case when process restart was caused by unhandled signal this condition
			 * isn't met and the full coredump is sent to analytics. To report other
			 * crashes not caused by signals we need to manually call coredump app together
			 * with controller event. */

			const char *process_name = strrchr(a->argv[0], '/') ? strrchr(a->argv[0], '/') + 1 : a->argv[0];
			const char *event_cmd_str = "syswrapper.sh service-restarted inittab %s status %d %d %d";
			const char *coredump_cmd_str = "cat /dev/null | /sbin/coredump %s %d %d %d %ld";

			const int event_cmd_len = strlen(event_cmd_str) - 8 + strlen(process_name) + 36;
			const int coredump_cmd_len = strlen(coredump_cmd_str) - 8 + strlen(process_name) + 48;

			const int buf_len = event_cmd_len > coredump_cmd_len ? event_cmd_len : coredump_cmd_len;
			char sys_cmd[buf_len];

			snprintf(sys_cmd, buf_len, event_cmd_str, process_name, ret, proc->pid, a->crash_count);
			if (system(sys_cmd) < 0)
				ERROR("Failed to execute %s: %m\n", sys_cmd, errno);

			snprintf(sys_cmd, buf_len, coredump_cmd_str, process_name, proc->pid, WTERMSIG(ret), a->crash_count, runtime);
			if (system(sys_cmd) < 0)
				ERROR("Failed to execute %s: %m\n", sys_cmd, errno);
	...
	list_for_each_entry_safe(action, next_action, &actions, list) {
		...
		if (remove) {
			...
			if (pid > 0) {
				...
				uloop_timeout_set(&action->tout, PROCESS_TERMINATION_TIMEOUT);
				kill(pid, SIGTERM);
			} else {
				free_action(action);
			}
