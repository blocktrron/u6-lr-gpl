...
static void mtk_phy_link_adjust(struct net_device *dev)
{
	...
	mtk_setup_hw_path(eth, mac->id, mac->phymode);
...
static int mtk_phy_connect(struct net_device *dev)
{
	...
	mac->phymode = of_get_phy_mode(np);
...
static inline struct mtk_rx_ring *mtk_get_rx_ring(struct mtk_eth *eth)
{
	int i;
		       ...
		       struct mtk_eth *eth)
{
	...
	while (likely(done < budget)) {
		...
		/* prefetch two indexes ahead */
		prefetch_idx = NEXT_RX_DESP_IDX(idx + 1, ring->dma_size);
		prefetch(&ring->dma[prefetch_idx]);
		prefetch(ring->data[prefetch_idx]);
...
static int __init mtk_init(struct net_device *dev)
{
	...
	mac_addr = of_get_mac_address(eth->dev->of_node);
	if (mac_addr)
